...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 *  -response: 요청에 대한 응답할때 필요한 객체
		 *  
		 *  <Get 방식과 Post 방식>
		 *  - Get 방식 : 사용자가 입력한 값들이 url 에 노출 / 데이터 길이제한 있음 / 대신 즐겨찾기 기능 사용할 때 편리
		 *  - Post 방식 : 사용자가 입력한 값들이 url에 노출 x / 데이터 길이제한 없음 / 대신 즐겨찾기 기능이 불편함 / Timeout이 존재
		 *  
		 */
		...
		// 2) 요청 시 전달된 값들을 뽑기 (request의 parameter영역으로부터)
		...
		if(saveId != null && saveId.equals("y")) {
			// 아이디를 저장하겠다.=> saveId 라는 키값으로 넘겨받은 아이디값을 쿠키로 저장
			
			//Cookie : 사이트가 사용하고 있는 서버에서 사용자의 컴퓨터에 저장하는 정보
			//         주로 보안과 관련없는 기능을 다룰 때 사용한다.

			/*
			 * 쿠키 생성
			 * - 쿠키는 객체를 생성한 다음 응답정보에 첨부해야 완전히 생성된다.
			 * - 쿠키 생성 시 name, value 는 필수 (키-밸류 세트로 입력)
			 * - expires (만료기간) 은 옵션  (Expires/Max-Age)
			 * - name, value 가 모두 문자열이여야 한다.
			 * - name 이 중복될 시 덮어씌워짐
			 */
			Cookie cookie = new Cookie("saveId", userId );
			cookie.setMaxAge(1 *24* 60 *60); //만료시간 1일 (초단위 작성)
			response.addCookie(cookie);
			
				
		}
		else { 
			// 아이디를 저장하지 않겠다.=>saveId 라는 키값을 가진 쿠키를 삭제
		...
		if(loginUser == null) { //로그인 실패=> 에러페이지 응답 (포워딩방식)
		   ...
		   *               => 현재 내가 작성중인 servlet의 url 맴핑값이 유지됨
		   ...
		   *  2. session : session에 담은 데이터는 모든 jsp와 servlet에서 꺼내다 쓸 수 있음
		   ...
		   *  
		   *  *객체들의 사용법
		   *  값을 담을때 : 객체명.setAttribute("키",밸류); =>Object 형식의 밸류
		  ...
		  // 4_1_3) 로그인이 성공했을 경우 보여질 성공메세지를 session에 담기
	    ...
	    view.forward(request, response);
		 ...
		 // 해당 선택된 jsp가 보여질 뿐 url에는 여전히 현재 이 서블릿 맵핑값이 남아있음
