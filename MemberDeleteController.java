...
public class MemberDeleteController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2) 요청시 전달값을 뽑아서 변수에 기록 및 VO 객체로 가공
		// 탈퇴하고자 하는 회원의 아이디,탈퇴하고자 하는 회원의 비밀번호
		 ...
		 if(result > 0 ) { //성공 => 메인페이지 응답 (로그아웃 되도록)
			
		   // 알람 띄우기	 
		   session.setAttribute("alertMsg", "성공적으로 회원 탈퇴 되었습니다. 그동안 이용해주셔서 감사합니다.");	 
		
		   // 로그아웃 => invalidate 메소드 활용(적합하지 않은 방식)
		  // session.invalidate();
		   
		  //invalidate() 메소드를 사용시 세션이 아예 만료되어
		   // alertMsg 메세지 자체도 무효화 됨 => alert가 되지 않음
		   // removeAttribute("키값")을 이용해서
		   // 현재 로그인한 사용자의 정보만 지워주는 방식으로 로그아웃을 시킴
